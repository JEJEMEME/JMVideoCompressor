...
// Helper extension for URL size calculation (assuming it exists elsewhere or add it here)
extension URL {
...
// Protocol definition for VideoFrameReducer (assuming it exists elsewhere or add it here)
...
public struct ReduceFrameEvenlySpaced: VideoFrameReducer {
    ...
    public func reduce(originalFPS: Float, to targetFPS: Float, with videoDuration: Float) -> [Int]? {
        ...
        let totalOriginalFrames = Int(ceil(originalFPS * videoDuration))
        let totalTargetFrames = Int(round(targetFPS * videoDuration))
